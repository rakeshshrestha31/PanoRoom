...
if __name__=='__main__':
    ...
    begin_tms = time.time()
    # process data in parallel, use all available cores
    from joblib import Parallel, delayed
    Parallel(n_jobs=-1)(delayed(parse_single_scene)(input_root_dir=os.path.join(args.output_dir, scene_folder), output_dir=os.path.join(args.output_dir, scene_folder, 'panorama')) for scene_folder in scene_folders_lst)
    
    end_tms = time.time()
    print(f'Processing time: {end_tms - begin_tms} seconds')
    # total_rooms_num = 0
    # total_cams_num = 0
